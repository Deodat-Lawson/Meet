...
const SETTINGS_FILE = node_path_1.default.join(electron_1.app.getPath('userData'), 'settings.json');
function readSettings() {
    try {
        const parsed = JSON.parse(node_fs_1.default.readFileSync(SETTINGS_FILE, 'utf8'));
...
function writeSettings(settings) {
    try {
        ...
        node_fs_1.default.writeFileSync(SETTINGS_FILE, JSON.stringify(settings, null, 2));
    }
    catch (error) {
...
function createWindow() {
    ...
    mainWindow.webContents.on('did-fail-load', (_e, code, description, failedUrl) => {
        if (code === -3)
            return; // aborted by a redirect, not a real failure
        void electron_1.dialog.showMessageBox({
            type: 'error',
            message: 'Could not reach the Team Studio server',
            detail: `${failedUrl}\n\n${description} (${code})\n\nCheck the server address in Team Studio ▸ Settings, or your network connection.`,
            buttons: ['OK'],
        });
    });
...
function installDisplayMediaHandler() {
    ...
    ses.setDisplayMediaRequestHandler(async (_request, callback) => {
        ...
        try {
            ...
            const chosen = await showPicker(sources.map((s) => ({
                id: s.id,
                name: s.name,
...
function showPicker(sources) {
    return new Promise((resolve) => {
        if (pickerWindow && !pickerWindow.isDestroyed())
            pickerWindow.close();
        pickerWindow = new electron_1.BrowserWindow({
            width: 760,
            height: 560,
        ...
        const finish = (id) => {
            if (settled)
                return;
            settled = true;
            electron_1.ipcMain.removeHandler('picker:sources');
            electron_1.ipcMain.removeAllListeners('picker:choose');
            resolve(id);
            if (pickerWindow && !pickerWindow.isDestroyed())
                pickerWindow.close();
            pickerWindow = null;
        };
        electron_1.ipcMain.handle('picker:sources', () => sources);
        electron_1.ipcMain.on('picker:choose', (_event, id) => finish(id));
        pickerWindow.on('closed', () => finish(null));
        void pickerWindow.loadFile(node_path_1.default.join(__dirname, '..', 'static', 'picker.html'));
    });
}
/* ----------------------------------------------------------------- menu */
function buildMenu() {
        ...
        { label: 'Window', submenu: [{ role: 'minimize' }, { role: 'zoom' }, { role: 'close' }] },
        {
            ...
            submenu: [
                ...
                },
                {
                    ...
                    click: () => void electron_1.shell.openExternal('https://github.com/Deodat-Lawson/Meet'),
                },
            ],
        },
    ];
...
async function promptForServer() {
    const result = await electron_1.dialog.showMessageBox({
        type: 'question',
        message: 'Team Studio server address',
 ...
 */
async function checkScreenPermission(interactive) {
    ...
    if (status === 'granted') {
        if (interactive) {
            await electron_1.dialog.showMessageBox({
                ...
                detail: 'Team Studio can share your screen.',
                buttons: ['OK'],
            });
        }
        return;
    ...
    const result = await electron_1.dialog.showMessageBox({
        ...
        detail: 'macOS requires explicit permission before an app can share your screen.\n\n' +
            'Open System Settings ▸ Privacy & Security ▸ Screen & System Audio Recording, enable Team Studio, then quit and reopen the app.',
        buttons: ['Open System Settings', 'Later'],
        defaultId: 0,
    });
    if (result.response === 0) {
        void electron_1.shell.openExternal('x-apple.systempreferences:com.apple.preference.security?Privacy_ScreenCapture');
    }
}
/* ------------------------------------------------------------- lifecycle */
