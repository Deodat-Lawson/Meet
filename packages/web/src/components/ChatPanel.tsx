  ...
  return (
    <aside className="panel">
      <div className="panel-header">
        ...
        <button className="icon-btn" onClick={() => setPanel('none')} aria-label="Close chat">
          <CloseIcon />
        </button>
      </div>
      ...
      <div className="panel-body" ref={listRef}>
        {room.chat.length === 0 ? (
          <div className="empty-state">
            <ChatIcon size={26} />
            No messages yet.
            <span style={{ fontSize: 12 }}>Messages are visible to everyone in the meeting.</span>
          </div>
        ) : (
              ...
              return (
                <div key={message.id}>
                  <div className="chat-msg-head">
                    ...
                    {recipient && <span className="chat-private">privately to {recipient}</span>}
      ...
      <div className="panel-footer">
        {peers.length > 0 && (
          <select className="select" value={to} onChange={(event) => setTo(event.target.value)} style={{ marginBottom: 8, height: 34, fontSize: 13 }}>
            <option value="">To: Everyone</option>
            {peers.map((peer) => (
              <option key={peer.id} value={peer.id}>
                To: {peer.displayName} (private)
              </option>
            ))}
          </select>
        )}
        <div className="chat-input-row">
          ...
          />
          <button className="btn btn-primary" onClick={() => void send()} disabled={!text.trim() || sending} aria-label="Send message">
            <SendIcon />
          </button>
        </div>
      </div>
